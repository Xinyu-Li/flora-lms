...
function local_boostnavigation_extend_navigation(global_navigation $navigation) {
    ...
    if (isset($config->insertcustomnodesusers) && ($COURSE->id == 5) && !empty($config->insertcustomnodesusers)) {
        // If yes, do it.
    ...
    if (isset($config->insertcustomcoursenodesusers) && ($COURSE->id == 5) && !empty($config->insertcustomcoursenodesusers)) {
        // Only proceed if we are inside a course and we are _not_ on the frontpage.
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID && $coursehomenode) {
            // If yes, do it.
    ...
    if (isset($config->insertcustombottomnodesusers) && ($COURSE->id == 5) && !empty($config->insertcustombottomnodesusers)) {
        // If yes, do it.
